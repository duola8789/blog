...
JavaScript在创建变量时自动进行了内存分配，并且在不适用时自动释放。释放的过程叫做垃圾回收。
...
这种算法没有办法处理循环引用的情况，例如下面的例子，两个对象被创建，并相互引用，形成循环。它们被调用后不再被外部引用，没用了，可以被回收了。但是引用计数算法认为它们都还至少又一次引用，所以不会被回收，一直保存在内存中。
